          ...
          <section className="hidden md:flex flex-row w-fit">
              {
                menus.map((menu, index) => (
                  <Link href={menu.link} about={menu.name} title={menu.name} key={index} className={`hover:bg-secondary hover:rounded-full px-5 py-2 duration-800 transition-all hover:text-primary ${menu.name == isSelected ? 'bg-secondary text-primary rounded-full' : 'bg-transparent'} ${isDetail? 'text-primary' : menu.name == isSelected ? 'bg-secondary text-primary rounded-full' : 'text-white'}`} onClick={(e) => setIsSelected(menu.name)}>
